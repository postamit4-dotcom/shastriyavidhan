...
const staticNodes = [
  ...
  node({
    ...
    label: "Ganesh Chaturthi Puja 2026",
    shortLabel: "Ganesh Chaturthi",
    href: "/ganesh-chaturthi-puja",
    pageType: "service",
    parentId: "puja-services",
    categories: ["festival-pujas", "hanuman-ganesh"],
    evidenceState: "verified",
    showInHtmlSitemap: true,
    priority: "P0",
    lastModified: "2026-09-13",
    image: "/images/ganesh-chaturthi-puja.jpg",
    notes: "Comprehensive Ganesh Chaturthi Puja 2026 dedicated guide, vidhi, and booking page.",
  }),
  node({
    id: "how-it-works",
    label: "How It Works",
  ...
  node({
    id: "pandit-registration",
    label: "Pandit Registration",
    href: "/pandit-registration",
    pageType: "partner_registration",
    parentId: "pandit-standards",
    evidenceState: "partially_verified",
    showInFooter: true,
    priority: "P1",
    lastModified: "2026-09-06",
    notes:
      "Public partner application page; private submissions are stored server-side and no document uploads are collected.",
  }),
  node({
    id: "acharya-sursain-brijwasi",
    label: "Acharya Sursain Brijwasi",
    shortLabel: "Sursain Brijwasi",
    ...
    notes:
      "Named Pandit Ji profile with owner-provided public contact details and photo; personal credentials remain intentionally limited to verified facts.",
  }),
  node({
    id: "acharya-sursain-brijwasi-raj-nagar-extension",
    label: "Acharya Sursain Brijwasi - Raj Nagar Extension",
    shortLabel: "Raj Nagar Extension Pandit Ji",
    href: "/pandit-ji/acharya-sursain-brijwasi-raj-nagar-extension-ghaziabad",
    pageType: "local_booking",
    parentId: "acharya-sursain-brijwasi",
    evidenceState: "partially_verified",
    showInHtmlSitemap: true,
    priority: "P1",
    lastModified: "2026-09-10",
    notes:
      "Local booking page for Raj Nagar Extension enquiries with service, quote, society access, and evidence-handling guidance.",
  }),
  node({
  ...
  node({
    id: "migration-notes",
    label: "Site Migration Notes",
    href: "/migration-notes",
    pageType: "site_notice",
...
const categoryNodes = serviceCategories.map((category) => {
  ...
  return node({
    ...
    lastModified: currentDate,
    notes: indexable
      ? `${services.length} linked services are available from this category page.`
      : `${services.length} linked service(s); category availability depends on service detail readiness.`,
  });
});
...
const locationNodes = locationPages
  ...
  .map((location) => {
    const publishedLocationSlugs = new Set(["ghaziabad", "noida"]);
    const publishedLocationDates = {
      ghaziabad: "2026-09-09",
      noida: "2026-09-10",
    };
    const publishedLocationNotes = {
      ghaziabad:
        "Ghaziabad location page is published because it has verified local profile linkage, service request copy, and cautious availability language.",
      noida:
        "Noida location page is published after local booking copy, cautious coverage language, Noida-mapped service selection, enquiry CTAs, and sitemap validation.",
    };
    const isPublishedLocation = publishedLocationSlugs.has(location.slug);
    ...
    return node({
      ...
      pageType: "location",
      parentId: "locations",
      publicationState: isPublishedLocation ? "live" : "conditional",
      evidenceState: "partially_verified",
      indexable: isPublishedLocation,
      showInFooter: false,
      showInHtmlSitemap: isPublishedLocation,
      priority: isPublishedLocation ? "P0" : "P2",
      lastModified: isPublishedLocation ? publishedLocationDates[location.slug] : currentDate,
      notes: isPublishedLocation
        ? publishedLocationNotes[location.slug]
        : "Location page exists for direct review but stays out of the XML sitemap until service coverage is owner-verified.",
    });
  });
...
export const serviceMegaMenuGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Register as a Pandit Ji", href: "/pandit-registration" },
