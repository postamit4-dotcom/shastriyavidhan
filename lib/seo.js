...
export function videoObjectJsonLd(video, pagePath) {
  if (!video?.id || !video.cardTitle || !video.summary || !video.thumbnailUrl || !video.uploadDate || !video.embedUrl) {
    return null;
  }

  return {
    "@type": "VideoObject",
    "@id": `${absoluteUrl(pagePath)}#video-${video.id}`,
...
export function servicePageJsonLd(service) {
  ...
  const graph = [
    ...
    breadcrumbJsonLd(serviceBreadcrumbItems(service), path),
  ];

  const videoObjects = Array.isArray(service.videoGuidance)
    ? service.videoGuidance.map((video) => videoObjectJsonLd(video, path)).filter(Boolean)
    : [];
